...
function* handleTemperatureUnitChange() {
  const location: string = yield select((state: RootState) => state.weather.currentWeather?.name);
  const weatherUnit: "metric" | "imperial" = yield select((state: RootState) => state.weather.unit);
...
export default function* rootSaga() {
  yield all([watchFetchWeather(), watchTemperatureUnitChange()]);
}
